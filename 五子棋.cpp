...
int flag = 0;
int ChessMap[25][25] = { 0 };//记录棋子数组

//初始化游戏：
void initGame() {
	initgraph(474, 711);//创建画面
	loadimage(NULL, L"bg.jpg");//载入背景图片
	...
	outtextxy(270, 500, L"玩家1:黑棋");
	outtextxy(130, 500, L"玩家2:白棋");
	...
	while (1) {
		...
		//如果鼠标左键按下
			if (ChessMap[MapX][MapY] != 0) {
				MessageBox(hwnd, L"这里有棋子了！请重新选择。", L"五子棋", MB_OK);
				continue;
			}
			if (flag % 2 == 0) {
		...
		if (judge(MapX, MapY)) {
			if (1 == flag % 2) {
				MessageBox(hwnd, L"玩家1;黑棋胜利", L"Game Over", MB_OK);
				return;
			}
			else {
				MessageBox(hwnd, L"玩家2;白棋胜利", L"Game Over", MB_OK);
				return;
			}
		}
	}
